...
def bin_conversion(text, prefix=app.PREFIX, width=app.WIDTH):
    ...
    if prefix:
        ...
        pre = ""
    output = "".join(
        [pre + bin(int(x, 16))[2:].zfill(int(width)) + " " for x in text]
    )
    return output.strip()
...
def hex_conversion(text, prefix=app.PREFIX, width=app.WIDTH):
    ...
    if prefix:
        ...
        pre = ""
    output = "".join(
        [pre + hex(int(x, 16))[2:].zfill(int(width) // 4) + " " for x in text]
    )
    return output.strip()
...
def oct_conversion(text, prefix=app.PREFIX, width=app.WIDTH):
    ...
    if prefix:
        ...
        pre = ""
    output = "".join([pre + oct(int(x, 16))[2:].zfill(3) + " " for x in text])
    return output.strip()
